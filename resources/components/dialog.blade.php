...
<div
    x-data="{
        mog_dialog: @js($open),
        open() {
            this.mog_dialog = true
        },
        close() {
            this.mog_dialog = false
        },
        toggle() {
            this.mog_dialog = ! this.mog_dialog
        },
    }"
    x-effect="
        mog_dialog
    ...
    <div
        x-show="mog_dialog"
        :data-state="mog_dialog ? 'open' : 'closed'"
        class="data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 fixed inset-0 z-50 overflow-y-auto bg-black/80"></div>
    <div
        x-show="mog_dialog"
        :data-state="mog_dialog ? 'open' : 'closed'"
